-- Window.lua MODIFICADO — BlackRed Theme + Logo transparente no fundo
-- Substitua o arquivo original em: src/Components/Window.lua
-- Mudanças:
--   1. Logo rbxassetid://84427155383225 adicionada no fundo com transparência alta
--   2. Selector (barra de tab ativo) usa cor vermelha
--   3. Suporte ao novo tema "BlackRed"
...
return function(Config)
	...
	-- ── Selector lateral (barra vermelha da tab ativa) ──────────────────────
	local Selector = New("Frame", {
		Size = UDim2.fromOffset(4, 0),
		BackgroundColor3 = Color3.fromRGB(220, 38, 38), -- Vermelho fixo
	...
	-- ── Logo transparente no fundo ───────────────────────────────────────────
	-- Fica por cima do fundo acrílico mas atrás de todos os controles.
	-- ImageTransparency = 0.88 → bem visível como watermark sem atrapalhar.
	-- ScaleType = Fit mantém proporções sem cortar a logo.
	local BgLogo = New("ImageLabel", {
		Image = "rbxassetid://84427155383225",
		Size = UDim2.new(0, 120, 0, 120),
		Position = UDim2.new(1, -130, 1, -130), -- canto inferior direito
		AnchorPoint = Vector2.new(0, 0),
		BackgroundTransparency = 1,
		ImageTransparency = 0.88,   -- quase invisível; ajuste entre 0.82–0.92
		ScaleType = Enum.ScaleType.Fit,
		ZIndex = 1,                 -- atrás de todos os controles
	})
	...
	Window.Root = New("Frame", {
		...
		BgLogo,                   -- logo inserida antes dos controles (ZIndex baixo)
		Window.TabDisplay,
	...
	SizeMotor:onStep(function(values)
		...
		-- reposiciona logo no canto inferior direito ao redimensionar
		BgLogo.Position = UDim2.new(1, -130, 1, -130)
	end)
